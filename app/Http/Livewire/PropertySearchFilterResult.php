...
use DB;
class PropertySearchFilterResult extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $data['other_count'] = $this->getProperties()->count();
        $data['properties'] = $this->getProperties()->paginate(20);
        // $data['featured'] = $this->getProperties()->where('featured', '1')->paginate(10);
        // $data['boost'] = $this->getProperties()->where('boost', 1)->paginate(10);
        return view('livewire.property-search-filter-result', $data);
    }
    ...
    //     $properties = Property::where('status', 1)->select('properties.*','cities.name as city_name')
