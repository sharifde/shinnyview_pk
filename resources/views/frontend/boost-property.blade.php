                        ...
                        <form method="get" action="{{ route('properties-listing') }}" class="property-filters">
                            <div class="s-select s-large">
                                <select name="city" class="p-city search-select">
                                    ...
                                    @foreach ($cities as $c)
                                        <option value="{{ $c->id }}"
                                            @if (isset($cityid) && $cityid == $c->id) selected="selected" @endif>{{ $c->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        @if ($p->boost == true)
                            <div class="col-md-3 col-sm-4 col-xs-12">
                                <div class="default-card">
                                    <a href="{{ route('view-property', ['ptype' => $p->propertyType, 'stype' => $p->purpose, 'city' => $p->city, 'id' => $p->id, 'slug' => $p->slug]) }}"
                                        class=" b-shadow">
                                        <div class="image">
                                            <img src="{{ asset('properties/gallery/' . $p->image) }}"
                                                alt="{{ $p->name }}">
                                            ...
                                            @endif
                                            <div
                                                class="p-type {{ strtolower(str_replace(' ', '-', $p->property_type_name)) }}">
                                                {{ $p->property_type_name }} for {{ $p->purpose }} </div>
                                        </div>
                                        ...
                                        <div class="additional-info">
                                            <div class="a-features"
                                                @if (empty($p->bedroom)) style="width: 80%;" @endif
                                                title="{{ $p->size }} {{ $p->area }}"><i
                                                    class="fas fa-vector-square"></i> <span>{{ $p->size }}
                                                    {{ $p->area }}</span></div>
                                            @if ($p->bedroom)
                                                <div class="a-features"><i class="fas fa-bed"></i> {{ $p->bedroom }} Bed
                                                </div>
                                            @endif
                                            @if ($p->bath)
                                                <div class="a-features"><i class="fas fa-bath"></i> {{ $p->bath }}
                                                    Bath
                                                </div>
                                            @endif
                                        </div>
